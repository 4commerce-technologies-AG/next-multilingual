...
function getTitle(messages, values) {
    const titleMessage = messages.get(exports.TITLE_KEY_ID);
    const slugMessage = messages.get(exports.SLUG_KEY_ID);
    const applicableTitle = titleMessage !== undefined ? titleMessage : slugMessage;
    if (applicableTitle === undefined) {
        __1.log.warn(`unable to use ${(0, __1.highlight)('getTitle')} in ${(0, __1.highlightFilePath)(messages.sourceFilePath)} because keys with identifiers ${(0, __1.highlight)(exports.TITLE_KEY_ID)} and ${(0, __1.highlight)(exports.SLUG_KEY_ID)} were not found in messages file ${(0, __1.highlightFilePath)(messages.messagesFilePath)}`);
        return '';
    }
...
function getMessagesFilePath(filesystemPath, locale) {
    ...
    return `${filesystemPath}/index.${(0, __1.normalizeLocale)(locale)}${translationFileExt}`;
}
...
function handleMessages(babelifiedMessages, caller, locale) {
    ...
    if (!babelifiedMessages.keyValueObjectCollection[locale]) {
        __1.log.warn(`unable to use ${(0, __1.highlight)(caller)}() in ${(0, __1.highlightFilePath)(babelifiedMessages.sourceFilePath)} because no message file could be found at ${(0, __1.highlightFilePath)(messagesFilePath)}`);
    }
